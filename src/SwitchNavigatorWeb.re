...
module Create = (Config: SwitchConfig) => {
  ...
  /* Router */
  let router = ReactNavigationWeb.Core.switchRouter(routes, config);

  /* navigator */
  let navigator =
    ReactNavigationWeb.Core.createNavigator(
      ReactNavigationWeb.Switch.switchView,
      router,
      config,
    );

  /* Wrap SwitchNavigator with the AppContainer - temporary */
  let render = ReactNavigationWeb.Web.createBrowserApp(navigator);
};
